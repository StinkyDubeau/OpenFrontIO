...
const pages = [
  { title: 'IdleFront Project Chronicle', subtitle: 'Created 2026-09-09', body: [] },
];

const commits = execSync('git log --date=short --pretty="%h|%ad|%s" --max-count=180', {
  cwd: root,
  encoding: 'utf8',
})
  .trim()
...
const commitByDate = (ts) => {
  ...
  return commits[commits.length - 1] ?? { hash: 'N/A', date: 'n/a', subject: 'n/a' };
};
...
addPage('Project Positioning', sectionLines([
...
addPage('OpenFront baseline summary', sectionLines([
...
addPage('Timeline', sectionLines([
  ...commits.slice(0, 90).map((c) => `${c.date}  ${c.hash}  ${c.subject}`),
]));

addPage('Playtest and engineering notes', sectionLines([
...
addPage('Sentiment analysis (command tone)', sectionLines([
...
addPage('Implementation state and checkpoint context', sectionLines([
...
for (let i = 0; i < screenshotMeta.length; i++) {
  ...
  addPage(`Screenshot Evidence ${i + 1}`, sectionLines([
...
function drawTextBlock(ctxRef, paragraphs, opts = {}) {
  ...
  for (const para of paragraphs) {
    ...
    const lines = wrapParagraph(ctxRef, para, CONTENT_WIDTH - indent, font, indent);
...
for (const [pi, page] of pages.entries()) {
  ...
  const titleLine = page.title.includes('Screenshot Evidence') ? null : null;
  ...
  drawTextBlock(ctxRef, body, page.title.includes('Timeline') ? {
  ...
  ctxRef.fillText(`Page ${pageIndex}`, PAGE_WIDTH - MARGIN_X - 52, PAGE_HEIGHT - 24);
}

if (!fs.existsSync(path.dirname(outputPdf))) fs.mkdirSync(path.dirname(outputPdf), { recursive: true });
