...
function readImageDim(buf) {
  ...
  return { width: 600, height: 400, colorspace: '/DeviceRGB', filter: 'DCTDecode' };
}
...
const commits = execSync('git log --date=short --pretty="%h|%ad|%s" --max-count=80', { cwd: root, encoding: 'utf8' })
  .trim()
  .split(/\r?\n/)
  .filter(Boolean)
  .map((line) => {
...
const shotData = shots
  .map((filePath) => {
    ...
    const commit = commits.find((c) => new Date(c.date) <= mtime) || commits[commits.length - 1];
    return {
      filePath,
      buffer,
      mtext,
...
for (const [idx, shot] of shotData.entries()) {
  const imgId = addObj(`<< /Type /XObject /Subtype /Image /Width ${shot.width} /Height ${shot.height} /ColorSpace ${shot.colorspace} /BitsPerComponent 8 /Filter /${shot.filter} /Length ${shot.buffer.length} >>`, true, { streamData: shot.buffer });
  imageIds.push(imgId);
}
...
function addTextPage(title, lines) {
  ...
  const contentId = addObj(`<< /Length ${Buffer.byteLength(body)} >>`, true, { streamData: Buffer.from(body) });
  const pageId = addObj(`<< /Type /Page /Parent ${pagesId} 0 R /MediaBox [0 0 612 792] /Resources << /Font << /F1 ${fontId} 0 R >> >> /Contents ${contentId} 0 R >>`);
  pageIds.push(pageId);
}
...
for (const [i, shotId] of imageIds.entries()) {
  ...
  const pageId = addObj(`<< /Type /Page /Parent ${pagesId} 0 R /MediaBox [0 0 612 792] /Resources ${res} /Contents ${contentId} 0 R >>`);
  pageIds.push(pageId);
}

objects[pagesId - 1].content = `<< /Type /Pages /Kids [${pageIds.join(' 0 R ')} ] /Count ${pageIds.length} >>`;
...
console.log(`Includes ${commits.length} commits and ${shotData.length} screenshot pages.`);
