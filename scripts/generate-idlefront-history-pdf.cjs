...
function formatJpegDimensions(buf) {
  ...
  while (i < buf.length) {
    ...
    if (marker >= 0xc0 && marker <= 0xc3 && end <= buf.length && len >= 9) {
      return { width: buf.readUInt16BE(start + 3), height: buf.readUInt16BE(start + 1) };
    }
    i = end;
  }
...
const commits = execSync('git log --date=short --pretty="%h|%ad|%s" --max-count=140', {
  cwd: root,
  encoding: 'utf8',
...
for (const file of screenshotFiles) {
  ...
  shots.push({ file, buffer, width, height, captured: st.toISOString().slice(0, 10), commit: c?.hash ?? 'N/A', commitDate: c?.date ?? 'n/a', commitMsg: c?.subject ?? 'n/a' });
}
...
const pdf = new Pdf();
const fontObj = pdf.addObject('<< /Type /Font /Subtype /Type1 /BaseFont /Helvetica >>');
...
function appendSection(title, lines, imageInfo = null) {
  ...
  while (cursor < wrapped.length) {
    ...
    if (firstPage) {
      ...
      if (imageRef) {
        const scaleW = 500;
        const scale = Math.min(scaleW / imageInfo.width, 220 / imageInfo.height, 1);
        const w = imageInfo.width * scale;
        const h = imageInfo.height * scale;
        const yImageTop = 392;
        chunks.push(`BT /F1 10 Tf 0 0 0 rg 1 0 0 1 ${xImage} ${yImageTop + h + 16} Tm (${escapePdf('Screenshot evidence')}) Tj ET`);
        chunks.push('q');
        chunks.push(`${w.toFixed(2)} 0 0 ${h.toFixed(2)} ${xImage} ${yImageTop} cm`);
    ...
    chunks.push('ET');
    const contentId = pdf.addObject('<< /Length __LEN__ >>', Buffer.from(chunks.join('\n')));
    const resources = imageRef
      ? `<< /Font << /F1 ${fontObj} 0 R >> /XObject << /Im${imageRef} ${imageRef} 0 R >> >>`
      : `<< /Font << /F1 ${fontObj} 0 R >> >>`;

    pageIds.push(pdf.addObject(`<< /Type /Page /Parent ${pagesObj} 0 R /MediaBox [0 0 612 792] /Resources ${resources} /Contents ${contentId} 0 R >>`));
    firstPage = false;
  }
}
...
pdf.objects[pagesObj].dict = `<< /Type /Pages /Count ${pageIds.length} /Kids [${pageIds.join(' 0 R ')}] >>`;
