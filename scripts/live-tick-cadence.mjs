...
if(!gameID || !['install','read','remove','limit-route-logs','export-start'].includes(action))throw new Error('Usage: GAME_ID install|read|remove|limit-route-logs|export-start');
...
function receive(message,session=''){
  if(message.method==='NodeWorker.attachedToWorker')sessions.push(message.params.sessionId);
  if(message.method==='NodeWorker.receivedMessageFromWorker')receive(JSON.parse(message.params.message),message.params.sessionId);
  const key=session+':'+message.id,p=pending.get(key);
  if(p){pending.delete(key);clearTimeout(p.timer);message.error?p.reject(message.error):p.resolve(message.result);}
}
...
function send(method,params={},session=''){
  ...
  return new Promise((resolve,reject)=>{
    ...
    pending.set(key,{resolve,reject,timer:setTimeout(()=>{pending.delete(key);reject(new Error('Timeout '+method));},15000)});
    const message=JSON.stringify({id,method,params});
    ws.send(session?JSON.stringify({id:++serial,method:'NodeWorker.sendMessageToWorker',params:{sessionId:session,message}}):message);
  });
}
const install=`(()=>{
...
try{
  ...
  for(const session of sessions){
    const id=await send('Runtime.evaluate',{expression:'process.getBuiltinModule("worker_threads").workerData?.start?.gameID',returnByValue:true},session);
    if(id.result?.value!==gameID)continue;
    found=true;
    const response=await send('Runtime.evaluate',{expression:action==='install'?install:action==='remove'?remove:action==='limit-route-logs'?limitLogs:action==='export-start'?'process.getBuiltinModule("worker_threads").workerData.start':read,returnByValue:true},session);
    if(response.exceptionDetails)throw new Error(JSON.stringify(response.exceptionDetails));
    const data=response.result?.value;
    if(action==='export-start') {
      await mkdir('.dev-logs/profiles',{recursive:true});
      const filename='.dev-logs/profiles/'+gameID+'-start-'+Date.now()+'.json';
      ...
      await mkdir('.dev-logs/profiles',{recursive:true});
      const filename='.dev-logs/profiles/'+gameID+'-cadence-'+Date.now()+'.json';
      await writeFile(filename,JSON.stringify(data));
      const rows=data.rows.filter(r=>r[2]>0);
      const summary=values=>{
        const sorted=[...values].sort((a,b)=>a-b),q=p=>sorted[Math.max(0,Math.ceil(sorted.length*p)-1)];
        return {samples:sorted.length,mean:values.reduce((a,b)=>a+b,0)/values.length,p50:q(.5),p95:q(.95),p99:q(.99),max:sorted.at(-1)};
      };
      console.log(JSON.stringify({file:filename,totalSamples:data.count,intervalMs:summary(rows.map(r=>r[2])),workerMs:summary(rows.map(r=>r[3])),coreMs:summary(rows.map(r=>r[4])),betweenTurnsMs:summary(rows.map(r=>Math.max(0,r[2]-r[3]))),slowest:rows.sort((a,b)=>b[2]-a[2]).slice(0,12)},null,2));
    }else console.log(JSON.stringify(data));
  }
  if(!found)throw new Error('Requested simulation worker not found');
}finally{
  await send('NodeWorker.disable').catch(()=>{});
  await send('Runtime.evaluate',{expression:'setTimeout(()=>process.getBuiltinModule("inspector").close(),500);void 0'}).catch(()=>{});
  ws.close();
}
