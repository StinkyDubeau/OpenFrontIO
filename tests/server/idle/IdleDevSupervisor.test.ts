...
const config = { Workspace: 'C:/checkout/idlefront', ConfigPath: 'C:/config/dev.json' };
it('keeps Expo loopback-only and uses the permanent game endpoint', () => {
  ...
  expect(spec.env.EXPO_PUBLIC_GAME_URL).toBe('https://atlas-dev.sightings.today/?debug=1');
});
