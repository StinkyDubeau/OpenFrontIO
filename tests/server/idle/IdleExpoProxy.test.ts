...
import { createExpoPreviewProxy, expoPreviewPrefix } from '../../../scripts/idle-expo-proxy.mjs';
...
it('preserves Metro update protocol headers while rewriting the manifest', async () => {
  ...
  const get = vi.spyOn(http, 'get').mockImplementation((_url: any, _options: any, callback: any) => {
    queueMicrotask(() => {
      callback(incoming);
      incoming.end(JSON.stringify({ launchAsset: { url: 'http://localhost:8081/index.bundle?platform=ios' } }));
    });
  ...
  try {
    ...
    createExpoPreviewProxy('test')({ method: 'GET', headers: {} }, res, new URL('https://example.com' + prefix + '/'));
    await finished;
    expect(res.writeHead).toHaveBeenCalledWith(200, expect.objectContaining({
      'expo-protocol-version': '0', 'expo-sfv-version': '0', 'content-type': 'application/expo+json',
    }));
    const manifest = JSON.parse(res.end.mock.calls[0][0]!);
    expect(manifest.launchAsset.url).toContain('https://atlas-dev.sightings.today' + prefix);
...
it('does not intercept game routes and rejects unkeyed/debug/write requests', () => {
  ...
  expect(proxy({method:'GET'}, res, new URL('https://example.com/worlds'))).toBe(false);
  expect(res.writeHead).not.toHaveBeenCalled();
  for (const [method, route] of [['GET','/__expo/wrong/'], ['GET',prefix+'/open-debugger'], ['POST',prefix+'/'], ['GET',prefix+'/assets/%2e%2e/secret']]) {
    expect(proxy({method}, res, new URL('https://example.com'+route))).toBe(true);
